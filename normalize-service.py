...
Reads from RAW_LOG_PATH (file hoặc stdin), writes normalized JSON to OUTPUT_PATH
"""
...
# ── Paths (override bằng env var) ───────────────────────────────────────────
INPUT_FILE  = os.getenv("NORMALIZED_PATH", "/opt/soc_ai/logs/log_normalized.json")
OUTPUT_FILE = os.getenv("DEDUP_PATH",      "/opt/soc_ai/logs/log_dedup.json")
OFFSET_FILE = os.getenv("DEDUP_OFFSET",    "/opt/soc_ai/logs/.dedup_offset")
 
# Location prefixes để nhận diện log type
...
def normalize_windows_event(archive_evt: dict) -> Optional[dict]:
    ...
    return {
        ...
        "message":     extract_first_line(system.get("message", "")),
        "maliciousIP": None,
        "waf":         "",
        "flow":        "",
        "linuxEvent":  "",
...
# ── Fortinet FortiGate ────────────────────────────────────────────────────────
# Hỗ trợ cả traffic, utm/ips, utm/webfilter, event/vpn
# Format: key=value pairs (standard FortiGate syslog)

def parse_fortinet_kv(raw_line: str) -> dict:
    """Parse FortiGate key=value log line thành dict."""
    result = {}
    # Pattern: key="value with spaces" hoặc key=value_no_spaces
...
def outcome_from_fortinet_action(action: str, subtype: str) -> str:
    a = (action or "").lower()
    if a in ("accept", "allow", "tunnel-up", "passthrough"):
        return "allowed"
    if a in ("deny", "block", "dropped", "close", "reset", "blocked"):
        return "blocked"
    if a in ("monitor", "count"):
        return "monitor"
    return "unknown"
...
def normalize_fortinet_event(archive_evt: dict) -> Optional[dict]:
    """
    Normalize Fortinet FortiGate syslog (key=value format).
    Hỗ trợ: traffic, utm/ips, utm/webfilter, event/vpn
    """
    full_log_raw = archive_evt.get("full_log", "")
    if not full_log_raw or not isinstance(full_log_raw, str):
        return None

    # Phải chứa devname hoặc devid để nhận diện là FortiGate
    ...
    log_type_fg = kv.get("type", "")    # traffic, utm, event
    subtype     = kv.get("subtype", "") # forward, ips, webfilter, vpn
    ...
    proto_int  = to_int_safe(proto)
    proto_name = get_protocol_name(proto_int) if proto_int else proto

    # Message tổng hợp theo type
    if log_type_fg == "utm" and subtype == "ips":
        message = f"FortiGate IPS {action}: {kv.get('attack', 'unknown attack')} from {src_ip}"
        ...
        message = f"FortiGate {log_type_fg}/{subtype} {action}: {src_ip}:{src_port} -> {dst_ip}:{dst_port}"
    ...
    severity = level_map.get(kv.get("level", "").lower(), "low")
    ...
    return {
        ...
        "network": {
            "source_ip":        src_ip,
            "source_port":      to_int_safe(src_port),
            "country":          kv.get("srccountry", ""),
            "destination_ip":   dst_ip,
            "destination_port": to_int_safe(dst_port),
            "protocol":         proto_name,
            "method":           None,
        },
        "message":     message,
...
def normalize_event(archive_evt: dict) -> Optional[dict]:
    ...
    # 4. Fortinet: detect by key=value pattern in full_log
...
def follow_file(path):
    """Đọc file theo kiểu tail -F, yield từng dòng hoàn chỉnh."""
    f            = None
    last_inode   = None
    first_open   = True
    buf          = ""
    ...
    while True:
        try:
            ...
            print(f"[WARN] follow_file error: {e}")
            time.sleep(0.5)
